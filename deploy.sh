...
while IFS= read -r line; do
    ...
    tempdata=\"\$first_extract|^|\$nextDayMidday|^|\$lastUpdate|^|\$temperature °C|^|\$humidity %|^|\$precipitation mm\"\$'\n'
    echo \"\$tempdata\"
...
while [ \"\$correctlines\" = false ]; do
    if [ \"\$(wc -l < \"\$places\")\" -eq 356 ]; then
        correctlines=true
        if ! crontab -l | grep -q \"temp_deploy.sh update_weather\"; then
            (crontab -l 2>/dev/null; echo \"*/30 * * * * temp_deploy.sh update_weather\") | crontab -
        fi
    fi
    sleep 5
done

#Movies original deploy.sh file and renames it into setup.sh + renames temp_deploy.sh into deploy.sh
if [ -f \"\$rPath/temp_deploy.sh\" ] && [ -f \"\$rPath/deploy.sh\" ]; then mv \"\$rPath/deploy.sh\" \"\$rPath/assets/setup.sh\" && mv \"\$rPath/temp_deploy.sh\" \"\$rPath/deploy.sh\"; fi
...
function removecrontab(){
    [ -f "$NEW_DEPLOY" ] && crontab -l | grep -q "$NEW_DEPLOY" && \
    (crontab -l | grep -v "$NEW_DEPLOY") | crontab -
    sleep 1
}
